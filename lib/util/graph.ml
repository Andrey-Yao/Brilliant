...
module MakeUnlabelled(VI: VIngredient) = struct
  ...
  let del_vert g v =
    let preds_v, succs_v = find g v in
    let g' =
      VS.fold preds_v
        ~init:g ~f:(fun acc p ->
          let preds_p, succs_p = find acc p in
          VM.set acc ~key:p ~data:(preds_p, VS.remove succs_p v)) in
    let g'' =
      VS.fold succs_v
        ~init:g' ~f:(fun acc s ->
...
module MakeLabelled(VI: VIngredient)(EI: EIngredient) = struct
  ...
  let del_vert g v =
    ...
    let g' =
      ES.fold preds_v
        ~init:g ~f:(fun acc (e, p) ->
          let preds_p, succs_p = find acc p in
          VM.set acc ~key:p ~data:(preds_p, ES.remove succs_p (e, v))) in
    let g'' =
      ES.fold succs_v
